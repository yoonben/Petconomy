	...
	if(e.value == "강원") var changeItem = gangwondo;
	 ...
	 jQuery(document).ready(function () {
			...
			jQuery.each(hangjungdong.sido, function (idx, code) {
				...
				let smallregion = '${param.smallregion}';
				let selected = code.sido==smallregion?'selected':'';
				jQuery('#sido').append(fn_option(code.sido, code.codeNm, selected));
	            if(smallregion){
	                fn_sido_change(); 
	                jQuery('#sigugun').val('smallregion').prop('selected',true);   
...
//sido 변경시 시군구 option 추가
jQuery('#megaregion').change(function () {
	fn_small_change();
});


function fn_small_change(){
    jQuery('#smallregion').show();
    jQuery('#smallregion').empty();
    jQuery('#smallregion').append(fn_option('', '선택')); //
    jQuery.each(changeItem, function (idx, code) {
        if (jQuery('#megaregion > option:selected').val() == code.megaregion)
            jQuery('#smallregion').append(fn_option(code.smallregion, code.codeNm));
    });

    //세종특별자치시 예외처리
    //옵션값을 읽어 비교
    if (jQuery('#megaregion option:selected').val() == change[idx]) {
        jQuery('#smallregion').hide();
        //index를 이용해서 selected 속성(attr)추가
        //기본 선택 옵션이 최상위로 index 0을 가짐
        jQuery('#smallregion option:eq(1)').attr('selected', 'selected');
        //trigger를 이용해 change 실행
        jQuery('#smallregion').trigger('change');
    }
}
...
<select class="form-select" id="megaregion" name="megaregion"  onchange="filterByLocation()" style="width:100px; display:inline-block;">
...
<select class="form-select form-select-inline" name="smallregion" id="smallregion" style=" width:100px; display:inline-block;">
